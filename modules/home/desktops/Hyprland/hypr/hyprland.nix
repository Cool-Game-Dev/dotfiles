...
in
{
  ...
  config = lib.mkIf cfg.enable {
    wayland.windowManager.hyprland = {
      enable = true;
      plugins = lib.ifList [
        (ifElse: ifElse: pkgs.hyprlandPlugins.hypr-dynamic-cursors)
      ];
      ...
      settings = {
        ...
        exec-once = lib.ifList [
          ...
          "kdeconnectd" # starts the KDE connect daemon
        ];

        plugin = {
          dynamic-cursors = {
            enabled = cfg.plugins.dynamicCursor.enable;
            mode = "tilt";
            tilt.limit = 2500;
            shake.enabled = false;
          };
        ...
        general = {
          ...
          "col.inactive_border" = "rgb(${vauxhall.grey.alpha})";
